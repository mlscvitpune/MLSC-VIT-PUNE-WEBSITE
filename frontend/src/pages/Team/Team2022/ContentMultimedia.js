import React from 'react';
import Tilt from 'react-parallax-tilt';

function ContentMultimedia() {
  return (
    <div>
      <div className="our-team pt-2" id="contentmultimedia">
        Content and Multimedia
      </div>
      <div className="row">
                <div className="your-element col-12 col-md-13 text-center py-4">
          <Tilt
            {...{
              tiltAngleYInitial: 10,
              transitionEasing: 'cubic-bezier(.05,.1,.32,.99)',
            }}
          >
            {/* <div className="member-image m29 mx-auto"></div>
            <div className="hover_3d pt-3">
              <div className="name">Kaushal Gore</div>
              <div className="position">Multimedia Lead</div>
              <div>
                <ul className="list-inline">
                  <li className="list-inline-item social-icons">
                    <a href="https://twitter.com/home">
                      <i className="fab fa-twitter"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://www.linkedin.com/in/kaushal-gore-a97862157/">
                      <i className="fab fa-linkedin"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://github.com/Kaushal-Is-Here">
                      <i className="fab fa-github"></i>
                    </a>
                  </li>
                </ul>
              </div>
            </div> */}
            <div className="_card mx-auto">
                <div className="position">Multimedia Lead</div>
              <div className="member-image m29 mx-auto"></div>
              <div className="hover_3d pt-3">
              <div className="name">Kaushal Gore</div>
                <div>
                  <ul className="list-inline">
                    <li className="list-inline-item social-icons">
                      <a href="https://twitter.com/home">
                      <img src="/Images/Icons/twitter.png" width="38px" height="38px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://www.linkedin.com/in/kaushal-gore-a97862157/">
                      <img src="/Images/Icons/linkedin.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://github.com/Kaushal-Is-Here">
                      <img src="/Images/Icons/github.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </Tilt>
        </div>
      </div>
      <div className="row">
        <div className="your-element col-12 col-md-4 text-center py-4">
          <Tilt
            {...{
              tiltAngleYInitial: 10,
              transitionEasing: 'cubic-bezier(.05,.1,.32,.99)',
            }}
          >
            {/* <div className="member-image m30 mx-auto"></div>
            <div className="hover_3d pt-3">
              <div className="name">Arnav Mukkawar</div>
              <div className="position">Multimedia</div>
              <div>
                <ul className="list-inline">
                  <li className="list-inline-item social-icons">
                    <a href="https://twitter.com/home">
                      <i className="fab fa-twitter"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://www.linkedin.com/in/arnav-mukkawar">
                      <i className="fab fa-linkedin"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://github.com/arnavmukkawar30">
                      <i className="fab fa-github"></i>
                    </a>
                  </li>
                </ul>
              </div>
            </div> */}
            <div className="_card mx-auto">
                <div className="position">Multimedia</div>
              <div className="member-image m30 mx-auto"></div>
              <div className="hover_3d pt-3">
              <div className="name">Arnav Mukkawar</div>
                <div>
                  <ul className="list-inline">
                    <li className="list-inline-item social-icons">
                      <a href="https://twitter.com/home">
                      <img src="/Images/Icons/twitter.png" width="38px" height="38px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://www.linkedin.com/in/arnav-mukkawar">
                      <img src="/Images/Icons/linkedin.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://github.com/arnavmukkawar30">
                      <img src="/Images/Icons/github.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </Tilt>
        </div>

        <div className="your-element col-12 col-md-4 text-center py-4">
          <Tilt
            {...{
              tiltAngleYInitial: 10,
              transitionEasing: 'cubic-bezier(.05,.1,.32,.99)',
            }}
          >
            {/* <div className="member-image m31 mx-auto"></div>
            <div className="hover_3d pt-3">
              <div className="name">Shreyash Bele</div>
              <div className="position">Multimedia</div>
              <div>
                <ul className="list-inline">
                  <li className="list-inline-item social-icons">
                    <a href="https://twitter.com/shreyash_bele">
                      <i className="fab fa-twitter"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://www.linkedin.com/in/shreyash-bele-7b81631b5/">
                      <i className="fab fa-linkedin"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://github.com/shreyash-bele">
                      <i className="fab fa-github"></i>
                    </a>
                  </li>
                </ul>
              </div>
            </div> */}
            <div className="_card mx-auto">
                <div className="position">Multimedia</div>
              <div className="member-image m31 mx-auto"></div>
              <div className="hover_3d pt-3">
              <div className="name">Shreyash Bele</div>
                <div>
                  <ul className="list-inline">
                    <li className="list-inline-item social-icons">
                      <a href="https://twitter.com/shreyash_bele">
                      <img src="/Images/Icons/twitter.png" width="38px" height="38px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://www.linkedin.com/in/shreyash-bele-7b81631b5/">
                      <img src="/Images/Icons/linkedin.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://github.com/shreyash-bele">
                      <img src="/Images/Icons/github.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </Tilt>
        </div>

        <div className="your-element col-12 col-md-4 text-center py-4">
          <Tilt
            {...{
              tiltAngleYInitial: 10,
              transitionEasing: 'cubic-bezier(.05,.1,.32,.99)',
            }}
          >
            {/* <div className="member-image m32 mx-auto"></div>
            <div className="hover_3d pt-3">
              <div className="name">Archis Khuspe</div>
              <div className="position">Multimedia</div>
              <div>
                <ul className="list-inline">
                  <li className="list-inline-item social-icons">
                    <a href="http://twitter.com/archiskhuspe">
                      <i className="fab fa-twitter"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="http://linkedin.com/in/archiskhuspe">
                      <i className="fab fa-linkedin"></i>
                    </a>
                  </li>
                   <li className="list-inline-item social-icons">
                    <a href="http://github.com/archiskhuspe">
                      <i className="fab fa-github"></i>
                    </a>
                  </li> 
                </ul>
              </div>
            </div> */}
            <div className="_card mx-auto">
                <div className="position">Multimedia</div>
              <div className="member-image m32 mx-auto"></div>
              <div className="hover_3d pt-3">
              <div className="name">Archis Khuspe</div>
                <div>
                  <ul className="list-inline">
                    <li className="list-inline-item social-icons">
                      <a href="http://twitter.com/archiskhuspe">
                      <img src="/Images/Icons/twitter.png" width="38px" height="38px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="http://linkedin.com/in/archiskhuspe">
                      <img src="/Images/Icons/linkedin.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="http://github.com/archiskhuspe">
                      <img src="/Images/Icons/github.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </Tilt>
        </div>
      </div>


 <div className="row">
        <div className="your-element col-12 col-md-3 text-center py-4">
          <Tilt
            {...{
              tiltAngleYInitial: 10,
              transitionEasing: 'cubic-bezier(.05,.1,.32,.99)',
            }}
          >
            {/* <div className="member-image m33 mx-auto"></div>
            <div className="hover_3d pt-3">
              <div className="name">Ghanshyam Pandey</div>
              <div className="position">Multimedia</div>
              <div>
                <ul className="list-inline">
                  <li className="list-inline-item social-icons">
                    <a href="https://twitter.com/realpandeyg?t=Juh69cP-w09Asqfa5ckpqA&s=09">
                      <i className="fab fa-twitter"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://www.linkedin.com/in/ghanshyam-pandey-110147214">
                      <i className="fab fa-linkedin"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="http://github.com/ghanshyam2009">
                      <i className="fab fa-github"></i>
                    </a>
                  </li>
                </ul>
              </div>
            </div> */}
            <div className="_card mx-auto">
                <div className="position">Multimedia</div>
              <div className="member-image m33 mx-auto"></div>
              <div className="hover_3d pt-3">
              <div className="name">Ghanshyam Pandey</div>
                <div>
                  <ul className="list-inline">
                    <li className="list-inline-item social-icons">
                      <a href="https://twitter.com/realpandeyg?t=Juh69cP-w09Asqfa5ckpqA&s=09">
                      <img src="/Images/Icons/twitter.png" width="38px" height="38px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://www.linkedin.com/in/ghanshyam-pandey-110147214">
                      <img src="/Images/Icons/linkedin.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="http://github.com/ghanshyam2009">
                      <img src="/Images/Icons/github.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </Tilt>
        </div>


  <div className="your-element col-12 col-md-3 text-center py-4">
          <Tilt
            {...{
              tiltAngleYInitial: 10,
              transitionEasing: 'cubic-bezier(.05,.1,.32,.99)',
            }}
          >
            {/* <div className="member-image m48 mx-auto"></div>
            <div className="hover_3d pt-3">
              <div className="name">Rushikesh Dighole</div>
              <div className="position">Multimedia</div>
              <div>
                <ul className="list-inline">
                  <li className="list-inline-item social-icons">
                    <a href="https://twitter.com/DigholeRushi">
                      <i className="fab fa-twitter"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://www.linkedin.com/in/rushikesh-dighole-44714720b">
                      <i className="fab fa-linkedin"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://github.com/DigholeRushi">
                      <i className="fab fa-github"></i>
                    </a>
                  </li>
                </ul>
              </div>
            </div> */}
            <div className="_card mx-auto">
                <div className="position">Multimedia</div>
              <div className="member-image m48 mx-auto"></div>
              <div className="hover_3d pt-3">
              <div className="name">Rushikesh Dighole</div>
                <div>
                  <ul className="list-inline">
                    <li className="list-inline-item social-icons">
                      <a href="https://twitter.com/DigholeRushi">
                      <img src="/Images/Icons/twitter.png" width="38px" height="38px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://www.linkedin.com/in/rushikesh-dighole-44714720b">
                      <img src="/Images/Icons/linkedin.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://github.com/DigholeRushi">
                      <img src="/Images/Icons/github.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </Tilt>
        </div>






        <div className="your-element col-12 col-md-3 text-center py-4">
          <Tilt
            {...{
              tiltAngleYInitial: 10,
              transitionEasing: 'cubic-bezier(.05,.1,.32,.99)',
            }}
          >
            {/* <div className="member-image m34 mx-auto"></div>
            <div className="hover_3d pt-3">
              <div className="name">Dyuti Bobby</div>
              <div className="position">Content</div>
              <div>
                <ul className="list-inline">
                  <li className="list-inline-item social-icons">
                    <a href="https://mobile.twitter.com/Dyuti_B">
                      <i className="fab fa-twitter"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://www.linkedin.com/in/dyuti-bobby-322ab3205">
                      <i className="fab fa-linkedin"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://github.com/DyutiBobby">
                      <i className="fab fa-github"></i>
                    </a>
                  </li>
                </ul>
              </div>
            </div> */}
            <div className="_card mx-auto">
                <div className="position">Content</div>
              <div className="member-image m34 mx-auto"></div>
              <div className="hover_3d pt-3">
              <div className="name">Dyutti Bobby</div>
                <div>
                  <ul className="list-inline">
                    <li className="list-inline-item social-icons">
                      <a href="https://mobile.twitter.com/Dyuti_B">
                      <img src="/Images/Icons/twitter.png" width="38px" height="38px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://www.linkedin.com/in/dyuti-bobby-322ab3205">
                      <img src="/Images/Icons/linkedin.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://github.com/DyutiBobby">
                      <img src="/Images/Icons/github.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </Tilt>
        </div>

        <div className="your-element col-12 col-md-3 text-center py-4">
          <Tilt
            {...{
              tiltAngleYInitial: 10,
              transitionEasing: 'cubic-bezier(.05,.1,.32,.99)',
            }}
          >
            {/* <div className="member-image m35 mx-auto"></div>
            <div className="hover_3d pt-3">
              <div className="name">Tejas Desai</div>
              <div className="position">Content</div>
              <div>
                <ul className="list-inline">
                  <li className="list-inline-item social-icons">
                    <a href="https://twitter.com/cartoon5091">
                      <i className="fab fa-twitter"></i>
                    </a>
                  </li>
                  <li className="list-inline-item social-icons">
                    <a href="https://www.linkedin.com/in/tejas-desai-a70307215/">
                      <i className="fab fa-linkedin"></i>
                    </a>
                  </li>
                   <li className="list-inline-item social-icons">
                    <a href="https://github.com/Cartoon72">
                      <i className="fab fa-github"></i>
                    </a>
                  </li> 
                </ul>
              </div>
            </div> */}
            <div className="_card mx-auto">
                <div className="position">Content</div>
              <div className="member-image m35 mx-auto"></div>
              <div className="hover_3d pt-3">
              <div className="name">Tejas Desai</div>
                <div>
                  <ul className="list-inline">
                    <li className="list-inline-item social-icons">
                      <a href="https://twitter.com/cartoon5091">
                      <img src="/Images/Icons/twitter.png" width="38px" height="38px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://www.linkedin.com/in/tejas-desai-a70307215/">
                      <img src="/Images/Icons/linkedin.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                    <li className="list-inline-item social-icons">
                      <a href="https://github.com/Cartoon72">
                      <img src="/Images/Icons/github.png" width="35px" height="35px"></img>
                      </a>
                    </li>
                  </ul>
                </div>
              </div>
            </div>
          </Tilt>
        </div>
      </div>
    </div>
  );
}

export default ContentMultimedia;
